...
function RegistrationForm({ onNavigate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        // Parse the error map generated by your GlobalExceptionHandler
        const errorData = await response.json();
        
        if (typeof errorData === 'object' && errorData !== null) {
          // Combine all validation error messages cleanly
          const combinedErrors = Object.values(errorData).join(" | ");
      ...
      // If successful, database row details are returned
      const result = await response.json();
      setSuccess(`Account for ${result.username} created successfully! Redirecting to login...`);
